...
export default function MemberDetailPage({ params }: { params: { userId: string } }) {
  ...
  const notStartedCourses = courses.filter((c) => c.state === 'NOT_STARTED');

  const renderCourseRow = (c: any) => {
    const meta = STATE_META[c.state] || STATE_META.NOT_STARTED;
    return (
      <div key={c.courseId} className="px-5 py-4">
        ...
        <div className="w-full bg-slate-100 h-1.5 rounded-full mt-2 overflow-hidden">
          <div className={`${meta.bar} h-full rounded-full`} style={{ width: `${c.progressPercent}%` }} />
        </div>
      </div>
    );
  };
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <Card shadow="md" className="border border-[#eff4ff]">
                ...
                <div className="text-[11px] text-[#737686] mt-1">Toàn bộ khóa học đã xuất bản</div>
              </Card>

              <Card shadow="md" className="border border-[#eff4ff]">
                <div className="text-xs font-semibold text-blue-700 mb-1">Tiến độ trung bình</div>
                <div className="text-2xl font-bold text-[#2563EB]">{data?.summary?.avgProgress || 0}%</div>
                <div className="text-[11px] text-[#737686] mt-1">Trung bình các khóa học</div>
              </Card>

              <Card shadow="md" className="border border-[#eff4ff]">
                <div className="text-xs font-semibold text-green-700 mb-1">Bài học đã xem</div>
                <div className="text-2xl font-bold text-green-700">{data?.user?.completedLessons || 0} bài</div>
                <div className="text-[11px] text-[#737686] mt-1">Đạt chuẩn ≥ 85%</div>
              </Card>
            </div>
